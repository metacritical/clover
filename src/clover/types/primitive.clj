...
(defn emit-long [num]
  (str
"
define %struct.Obj* @clj_val(%struct._GHashTable*) #0 {
  %2 = alloca %struct._GHashTable*, align 8
  store %struct._GHashTable* %0, %struct._GHashTable** %2, align 8
  %3 = call %struct.Obj* @number(i64 " num ")
  ret %struct.Obj* %3
}
"))
...
(defn emit-string [strn]
  (let [len (inc (count strn))]
    (str
"
@.str = private unnamed_addr constant [" len " x i8] c\"" strn "\\00\", align 1
...
(defn emit-symbol [sym]
  (let [len (inc (count (str sym)))]
    (str
"
@.str = private unnamed_addr constant [" len " x i8] c\"" sym "\\00\", align 1
...
(defn emit-keyword [key]
  (let [len (inc (count (str ":" (name key))))]
    (str
"
@.str = private unnamed_addr constant [" len " x i8] c\"" key "\\00\", align 1
